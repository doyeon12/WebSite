document.addEventListener('DOMContentLoaded', function () {
    ...
    // 아이디엄 슬라이드 기능 관련 코드
    const idiomList = document.querySelector('.idiom-main li');
    const prevButton = document.querySelector('.idiom-main button:first-child');
    const nextButton = document.querySelector('.idiom-main button:last-child');
    ...
    function updateIdiom(index) {
        idiomList.textContent = idioms[index];
    }

    prevButton.addEventListener('click', function() {
        currentIndex = (currentIndex === 0) ? idioms.length - 1 : currentIndex - 1;
        idiomList.style.transform = 'translateX(-100%)';
        setTimeout(() => {
            updateIdiom(currentIndex);
            idiomList.style.transform = 'translateX(0)';
        }, 500);
    });

    nextButton.addEventListener('click', function() {
        currentIndex = (currentIndex === idioms.length - 1) ? 0 : currentIndex + 1;
        idiomList.style.transform = 'translateX(100%)';
        setTimeout(() => {
            updateIdiom(currentIndex);
            idiomList.style.transform = 'translateX(0)';
    ...
    updateIdiom(currentIndex); // 초기 텍스트 설정
});
